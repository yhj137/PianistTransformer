...
2.  **调整生成参数**:
    *   **`Temperature`**: 控制生成结果的随机性。值越高，演奏的"即兴"感越强。注意，温度过低时模型可能会产生极端的表达，陷入局部模式的重复。建议在默认值的基础上进行适当调整以获得较好的效果。
    *   **`Top-p`**: 一种更先进的采样策略，用于控制结果的多样性。通常建议保持默认值。
    *   **`最大速度`**: 用于控制可编辑渲染midi保存时的最大Tempo值，超过这个值的Tempo会被截断。
    *   **`踏板偏移`**: 一个后处理参数（范围：-1.0 到 +1.0），用于在单个音符内前后移动踏板控制变化事件的时间位置。每个音符携带4个等间距的踏板点位——①位于音符起始点，②③④均匀分布在该音符到下一音符的间隔中。负值将踏板变化向前（提前）移动，正值向后（延迟）移动。滑块下方有四个开关按钮（①–④），控制哪些踏板点位受偏移量影响。默认点位①固定不动，点位②–④受偏移控制。调整完成后，点击 **`应用`** 即可对当前活跃版本重新生成踏板时序，无需重新运行完整的模型推理。
