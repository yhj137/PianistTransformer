...
2.  **Adjust Generation Parameters**:
    ...
    *   **`Pedal Offset`**: A post-processing parameter (range: -1.0 to +1.0) that shifts the timing of pedal control change events within each note. In our representation, each note carries 4 equally-spaced pedal points — ① is at the note onset, ②③④ are evenly distributed over the interval to the next note. Negative values shift pedal changes earlier (forward), positive values shift them later (backward). Below the slider, four toggle buttons (①–④) control which pedal points are affected by the offset. By default, point ① is fixed in place and points ②–④ are shiftable. After adjusting, click **`Apply`** to regenerate the pedal timing for the currently active version — no need to re-run the full model inference.
